...
protocol AuthenticationProtocol {
    func register(data:Data?,name: String, email: String, password: String, phone: String,phoneCode:String, type: String,compeletion : @escaping(Result<BaseResponse<LoginModel>?,NSError>,ErrorResponse?)->Void)
...
extension AuthenticationProtocol  {
    ...
    func register(data:Data?,name: String, email: String, password: String, phone: String,phoneCode:String, type: String,compeletion : @escaping(Result<BaseResponse<LoginModel>?,NSError>,ErrorResponse?)->Void){
        ...
        auth.uploadPdf(parmss: [
            ...
            "type": type
        ], name: "center_details_in_file", image: data!, target: .register(name: name, email: email, password: password, phone: phone, phoneCode: phoneCode, type: type), responseClass: BaseResponse<LoginModel>.self, compeletion:compeletion)
        }else{
